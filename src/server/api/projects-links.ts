...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!entityTypePattern.test(body.entityType)) {
      return NextResponse.json(
        { error: 'Entity type must follow dotted namespace format (e.g., "crm.account", "marketing.plan")' },
        { status: 400 }
      );
    }
    ...
    const existing = await db
      ...
      .where(
        and(
          ...
          eq(projectLinks.entityType, body.entityType),
          eq(projectLinks.entityId, body.entityId)
        )
      )
      .limit(1);
    ...
    await logActivity(
      ...
      'project.link_added',
      user.sub,
      `Link to ${body.entityType}:${body.entityId} was added to project "${project.name}"`,
      { projectId, entityType: body.entityType, entityId: body.entityId }
    );
