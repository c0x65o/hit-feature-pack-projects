 ...
 */
function extractIds(request: NextRequest): { projectId: string | null; groupId: string | null } {
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!PROJECT_ROLES.includes(nextRole as (typeof PROJECT_ROLES)[number])) {
    ...
    const [updated] = await db
      ...
      .set({
        role: nextRole as (typeof PROJECT_ROLES)[number],
    ...
    if (existing.role !== updated.role) {
      ...
      await logActivity(
        ...
        user.sub,
        `Group ${groupId} role changed from "${existing.role}" to "${updated.role}" in project "${project?.name || projectId}"`,
        { projectId, groupId, fromRole: existing.role, toRole: updated.role }
      );
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await logActivity(
      ...
      user.sub,
      `Group ${groupId} was removed from project "${project?.name || projectId}"`,
      { projectId, groupId }
    );
