...
export async function PUT(request) {
    try {
        ...
        if (existing.isSystem && (body.isActive === false || body.isSystem === false)) {
            return NextResponse.json({ error: 'Cannot deactivate or modify system activity types' }, { status: 400 });
        }
        // Build update object
        const updates = {
            updatedAt: new Date(),
...
export async function DELETE(request) {
    try {
        ...
        if (existing.isSystem) {
            return NextResponse.json({ error: 'Cannot delete system activity types' }, { status: 400 });
        }
