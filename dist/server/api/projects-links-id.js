...
export async function GET(request) {
    try {
        ...
        return NextResponse.json({ error: 'Failed to fetch project link' }, { status: 500 });
    }
}
/**
...
export async function PUT(request) {
    try {
        ...
        const [link] = await db
            ...
            .returning();
        // Log activity
        await logActivity(db, projectId, 'project.link_updated', user.sub, `Link to ${link.entityType}:${link.entityId} was updated in project "${project?.name || projectId}"`, { projectId, entityType: link.entityType, entityId: link.entityId });
        return NextResponse.json({ data: link });
    }
    catch (error) {
...
export async function DELETE(request) {
    try {
        ...
        await db
            ...
            .where(and(eq(projectLinks.projectId, projectId), eq(projectLinks.id, linkId)));
        // Log activity
        await logActivity(db, projectId, 'project.link_removed', user.sub, `Link to ${existing.entityType}:${existing.entityId} was removed from project "${project?.name || projectId}"`, { projectId, entityType: existing.entityType, entityId: existing.entityId });
        return NextResponse.json({ success: true });
    }
    catch (error) {
        console.error('[projects] Remove link error:', error);
        return NextResponse.json({ error: 'Failed to remove project link' }, { status: 500 });
    }
}
