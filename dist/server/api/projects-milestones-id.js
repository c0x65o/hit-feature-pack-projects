...
export async function GET(request) {
    try {
        ...
        const [milestone] = await db
            ...
            .where(and(eq(projectMilestones.projectId, projectId), eq(projectMilestones.id, milestoneId)))
            .limit(1);
        if (!milestone) {
    ...
    try {
        ...
        if (body.status && body.status !== existing.status) {
            if (body.status === 'completed') {
                await logActivity(db, projectId, 'project.milestone_completed', user.sub, `Milestone "${milestone.name}" was completed for project "${project?.name || projectId}"`, activityMetadata);
            }
            else {
                await logActivity(db, projectId, 'project.milestone_updated', user.sub, `Milestone "${milestone.name}" status changed from "${existing.status}" to "${milestone.status}"`, { ...activityMetadata, oldStatus: existing.status, newStatus: milestone.status });
            }
        }
        else {
            await logActivity(db, projectId, 'project.milestone_updated', user.sub, `Milestone "${milestone.name}" was updated`, activityMetadata);
        }
        return NextResponse.json({ data: milestone });
    }
    catch (error) {
...
export async function DELETE(request) {
    try {
        ...
        await db
            ...
            .where(and(eq(projectMilestones.projectId, projectId), eq(projectMilestones.id, milestoneId)));
        // Log activity
        await logActivity(db, projectId, 'project.milestone_deleted', user.sub, `Milestone "${existing.name}" was deleted from project "${project?.name || projectId}"`, { projectId, milestoneId: existing.id, milestoneName: existing.name });
