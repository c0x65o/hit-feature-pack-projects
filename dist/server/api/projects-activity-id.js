...
import { projectActivity, projectActivityTypes, } from '@/lib/feature-pack-schemas';
...
export async function GET(request) {
    try {
        ...
        const [activity] = await db
            ...
            .where(and(eq(projectActivity.projectId, projectId), eq(projectActivity.id, activityId)))
            .limit(1);
        if (!activity) {
...
export async function PUT(request) {
    try {
        ...
        if (!existing.typeId) {
            return NextResponse.json({ error: 'System activities cannot be edited' }, { status: 400 });
        }
        // Build update object
        const updates = {};
        if (body.typeId !== undefined) {
            ...
            const [activityType] = await db
                ...
                .where(and(eq(projectActivityTypes.id, body.typeId), eq(projectActivityTypes.isActive, true)))
                .limit(1);
            if (!activityType) {
        ...
        return NextResponse.json({ error: 'Failed to update project activity' }, { status: 500 });
    }
}
/**
...
export async function DELETE(request) {
    try {
        ...
        if (!existing.typeId) {
            return NextResponse.json({ error: 'System activities cannot be deleted' }, { status: 400 });
        }
        // Delete activity
        await db
        ...
        return NextResponse.json({ error: 'Failed to delete project activity' }, { status: 500 });
    }
}
