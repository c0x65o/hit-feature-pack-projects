...
export async function POST(request) {
    try {
        ...
        const requestedOwnerGroupId = typeof body.ownerGroupId === 'string' && body.ownerGroupId.trim() ? body.ownerGroupId.trim() : null;
        const ownerGroupId = requestedOwnerGroupId || groupIds[0] || null;
        if (!ownerGroupId) {
            return NextResponse.json({ error: 'Project requires an owner group. Provide ownerGroupId or ensure the user has at least one group.' }, { status: 400 });
        }
        ...
        // Log activity
        await logActivity(db, project.id, 'project.created', user.sub, `Project "${project.name}" was created`, { projectId: project.id, projectName: project.name, ownerGroupId });
