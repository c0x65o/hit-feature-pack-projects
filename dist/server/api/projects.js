    ...
    try {
                ...
                for (const f of filters) {
                    ...
                    if (field === 'name') {
                        ...
                        if (operator === 'dateEquals')
                            ...
                            if (fromRaw)
                                viewFilterConditions.push(sql `${projects.lastUpdatedOnTimestamp}::date >= ${String(fromRaw)}`);
                            if (toRaw)
                                viewFilterConditions.push(sql `${projects.lastUpdatedOnTimestamp}::date <= ${String(toRaw)}`);
                        }
        ...
        if (isMetricSort) {
            ...
            const rev = db
                ...
                revenue: sql `coalesce(sum(${metricsMetricPoints.value})::float8, 0)`.as('revenue'),
            })
                .from(projects)
                .leftJoin(metricsLinks, and(eq(metricsLinks.linkType, 'metrics.field_mapper'), sql `coalesce(((metricsLinks as any).metadata ->> 'project_slug'), '') = ${projects.slug}`, sql `coalesce(((metricsLinks as any).metadata ->> 'steam_app_id'), '') <> ''`))
                .leftJoin(metricsMetricPoints, and(eq(metricsMetricPoints.metricKey, 'gross_revenue_usd'), sql `(${metricsMetricPoints.dimensions} ->> 'steam_app_id') = ((metricsLinks as any).metadata ->> 'steam_app_id')`, sql `1=1 ${timeFilter}`))
                .groupBy(projects.id)
                .as('rev_30d');
            metricJoin = rev;
